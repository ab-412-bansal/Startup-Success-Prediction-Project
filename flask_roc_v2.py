...
def roc_curve(m,d,l,n,c):
    if m == "model1":
        model = GradientBoostingClassifier(criterion = 'friedman_mse', learning_rate = float(c),
...
X = data[:,:-1]
y = data[:,9]
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.38775, random_state = 0)
